...
public class v_AIController : v_AIAnimator, IMeleeFighter
{
    [Header("--- Iterations ---")]
    ...
    bool CheckTargetIsAlive()
    {
        ...
        var vChar = target.GetComponent<Character>();
        if (vChar == null)
            return false;
    ...
    {
        var forward = AgentDone() ? (target != null && OnStrafeArea && canSeeTarget ? (new Vector3(destination.x, transform.position.y, destination.z) - transform.position) :transform.forward) : agent.desiredVelocity;
